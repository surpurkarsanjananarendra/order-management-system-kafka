...
// this is the wrapper function for the callback func so that db is upated for every callback
// closure of callback fun just bcz kafka knows only callback not the db and hence to make sure the db is updated for each callback
// we already have data in struct form in payload with address, just pass it as it is wihout &, it will work!!
func ConsumeOrder(ctx context.Context, db *gorm.DB) kafka.KeyValueCallback {
	return func(
		key string,
