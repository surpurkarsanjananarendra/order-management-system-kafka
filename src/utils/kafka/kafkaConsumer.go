...
func StartConsumer(topic string, callback func(map[string]any)) {
	...
	config.Consumer.Group.Session.Timeout = 20 * time.Second   // how long before consumer is considered dead
	config.Consumer.Group.Heartbeat.Interval = 6 * time.Second // heartbeat to coordinator
